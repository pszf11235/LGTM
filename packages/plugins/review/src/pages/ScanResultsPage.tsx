 ...
 *
 * Violations grouped by rule. Scrollable with viewport clipping.
 */
...
export function ScanResultsPage({ onStatusHint }: ScanResultsPageProps) {
  ...
  } = useScrollableList(violations, { reservedLines: 8 });

  useEffect(() => {
    onStatusHint("↑↓ navigate  d/u page  g/G top/bottom");
    loadResults();
  }, []);
  ...
  return (
    <Box flexDirection="column">
      ...
      {visibleItems.map((v) => {
        const isSelected = violations.indexOf(v) === selectedIdx;
