...
import type { PRRef } from "./review-store.js";
import { listReviewedPRs } from "./review-store.js";
import { loadWatchList } from "@lgtm/core/registry/watch-list.js";
...
export function parsePrRef(input: string): ParsedRef | RefError {
  ...
  if (url) {
    return { owner: url[1], repo: url[2], pr: Number(url[3]) };
  }

  // owner/repo#42 or owner/repo/42
  const qualified = raw.match(/^([\w.-]+)\/([\w.-]+)(?:#|\/)(\d+)$/);
  if (qualified) {
    return { owner: qualified[1], repo: qualified[2], pr: Number(qualified[3]) };
  }

  // #42 or 42
  const bare = raw.match(/^#?(\d+)$/);
  if (bare) {
    return { pr: Number(bare[1]) };
  }
 ...
 */
export function resolvePrRef(lgtmDir: string, input: string): PRRef | RefError {
  ...
  const reviewed = listReviewedPRs(lgtmDir).filter((r) => r.pr === parsed.pr);

  if (reviewed.length === 1) return reviewed[0];

  if (reviewed.length > 1) {
    return {
      error: `#${parsed.pr} is ambiguous, ${reviewed.length} repos have one`,
      candidates: reviewed,
    };
  }
  ...
  const watched = loadWatchList(lgtmDir);

  if (watched.length === 1) {
    return { owner: watched[0].owner, repo: watched[0].repo, pr: parsed.pr };
  }

  if (watched.length > 1) {
    return {
      error: `#${parsed.pr} has no review on disk, and ${watched.length} repos are watched, so it is unclear which you mean`,
      candidates: watched.map((w) => ({ owner: w.owner, repo: w.repo, pr: parsed.pr })),
    };
  }

  return {
    error: `#${parsed.pr} has no review on disk and no repos are watched. Use owner/repo#${parsed.pr}`,
  };
}
